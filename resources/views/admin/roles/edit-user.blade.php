<x-app-layout>
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 class="text-lg font-semibold text-gray-900">Edit roles: {{ $user->name }}</h2>
        <p class="mt-1 text-sm text-gray-500">{{ $user->email }}</p>

        <form action="{{ route('admin.roles.update-user', $user) }}" method="POST" class="mt-6">
            @csrf
            @method('PUT')
            <div class="space-y-2">
                @foreach($roles as $role)
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="roles[]" value="{{ $role->name }}" {{ $user->hasRole($role->name) ? 'checked' : '' }} class="rounded border-gray-300 text-violet-600 focus:ring-violet-500">
                    <span>{{ $role->name }}</span>
                </label>
                @endforeach
            </div>
